...
impl AccountBinding {
    pub fn generate(&self) {
        ...
        // by default, axia use sr25519 for account address
        let axia_address = generate_axia_address::<sp_core::sr25519::Pair>(seeds.phrase());

        println!("Seed phrase:      {}", seeds.phrase());
        println!("Axia address:   {}", axia_address);
        println!("EVM address:      0x{:x}", account);
        println!("EVM private key:  0x{:x}", evm_private_key);
    }
}

fn generate_axia_address<Pair>(seed: &str) -> String
