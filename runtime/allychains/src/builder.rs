...
pub(crate) struct Bench<T: paras_inherent::Config> {
	pub(crate) data: AllychainsInherentData<T::Header>,
...
impl<T: paras_inherent::Config> BenchBuilder<T> {
	...
	/// Maximum number of validators participating in allychains consensus (a.k.a. active validators).
	...
	/// Register `cores` count of allychains.
	///
	...
	fn setup_session(
		...
		assert_eq!(paras::Pallet::<T>::allychains().len(), total_cores as usize);
