...
/// Enumerates the phase in the setup process of a channel between two allychains.
enum AllychainSetupStep {
	...
	<T as frame_system::Config>::Origin: From<crate::Origin>,
{
	...
	// Make both a allychain if they are already not.
	if !Paras::<T>::is_allychain(sender) {
		register_allychain_with_balance::<T>(sender, deposit);
	}
	if !Paras::<T>::is_allychain(recipient) {
		register_allychain_with_balance::<T>(recipient, deposit);
	}
...
frame_benchmarking::benchmarks! {
	...
	hrmp_accept_open_channel {
		let [(sender, _), (recipient, recipient_origin)] =
			establish_para_connection::<T>(1, 2, AllychainSetupStep::Requested);
	...
	hrmp_close_channel {
		let [(sender, sender_origin), (recipient, _)] =
			establish_para_connection::<T>(1, 2, AllychainSetupStep::Established);
		let channel_id = HrmpChannelId { sender, recipient };
	...
	// NOTE: a single allychain should have the maximum number of allowed ingress and egress
	// channels.
	force_clean_hrmp {
		// ingress channels to a single leaving allychain that need to be closed.
		let i in 0 .. (HRMP_MAX_INBOUND_CHANNELS_BOUND - 1);
		// egress channels to a single leaving allychain that need to be closed.
		let e in 0 .. (HRMP_MAX_OUTBOUND_CHANNELS_BOUND - 1);

		// first, update the configs to support this many open channels...
		assert_ok!(
			Configuration::<T>::set_hrmp_max_allychain_outbound_channels(frame_system::RawOrigin::Root.into(), e + 1)
		);
		assert_ok!(
			Configuration::<T>::set_hrmp_max_allychain_inbound_channels(frame_system::RawOrigin::Root.into(), i + 1)
		);
		// .. and enact it.
	...
	force_process_hrmp_open {
		...
		for id in 0 .. c {
			let _ = establish_para_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, AllychainSetupStep::Accepted);
		}
	...
	force_process_hrmp_close {
		...
		for id in 0 .. c {
			let _ = establish_para_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, AllychainSetupStep::CloseRequested);
		}
	...
	hrmp_cancel_open_request {
		...
		for id in 0 .. c {
			let _ = establish_para_connection::<T>(PREFIX_0 + id, PREFIX_1 + id, AllychainSetupStep::Requested);
		}
	...
	// worse case will be `n` allychain channel requests, where in all of them either the sender or
