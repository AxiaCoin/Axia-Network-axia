...
pub struct AxiaPrecompiles<R>(PhantomData<R>);

impl<R> AxiaPrecompiles<R>
where
	R: pallet_evm::Config,
{
	pub fn new() -> Self {
...
impl<R> PrecompileSet for AxiaPrecompiles<R>
where
	R: pallet_evm::Config,
{
	fn execute(
		...
		match address {
			...
			// Non-Axia specific nor Ethereum precompiles :
			a if a == hash(1024) => {
