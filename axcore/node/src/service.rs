...
use fc_consensus::AxiaBlockImport;
use fc_mapping_sync::{MappingSyncWorker, SyncStrategy};
use fc_rpc::EthTask;
use fc_rpc_core::types::{FeeHistoryCache, FilterPool};
use axcore_runtime::{self, opaque::Block, RuntimeApi, SLOT_DURATION};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		axcore_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		axcore_runtime::native_version()
	}
}
...
#[cfg(feature = "manual-seal")]
pub type ConsensusResult = (
	AxiaBlockImport<Block, Arc<FullClient>, FullClient>,
	Sealing,
);
...
> {
	...
	#[cfg(feature = "manual-seal")]
	{
		...
		let frontier_block_import =
			AxiaBlockImport::new(client.clone(), client.clone(), frontier_backend.clone());
	...
	#[cfg(feature = "aura")]
	{
		...
		let frontier_block_import = AxiaBlockImport::new(
...
pub fn new_full(mut config: Configuration, cli: &Cli) -> Result<TaskManager, ServiceError> {
	...
	// Spawn Axia EthFilterApi maintenance task.
	...
	// Spawn Axia FeeHistory cache maintenance task.
	task_manager.spawn_essential_handle().spawn(
