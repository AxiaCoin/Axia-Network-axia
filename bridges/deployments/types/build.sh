...
# (Rialto<>Millau and BetaNet<>Wococo at the moment).
#
...
# Create types for our supported bridge pairs (Rialto<>Millau, BetaNet<>Wococo)
...
jq -s '.[0] * .[1] * .[2]' betanet-wococo.json common.json betanet.json > ../types-betanet.json
jq -s '.[0] * .[1] * .[2]' betanet-wococo.json common.json wococo.json > ../types-wococo.json
