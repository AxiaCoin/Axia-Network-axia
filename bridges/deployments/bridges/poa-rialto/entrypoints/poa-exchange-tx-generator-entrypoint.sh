...
#
# This scripts periodically calls relay binary to generate PoA -> Axlib
# exchange transaction from hardcoded PoA senders (assuming they have
# enough funds) to hardcoded Axlib recipients.
...
# All possible Axlib recipients (hex-encoded public keys)
SUB_RECIPIENTS=(
	# Alice (5GrwvaEF...)
...
do
	...
	# submit transaction
	echo "Sending $EXCHANGE_AMOUNT_ETH from PoA:$ETH_SIGNER to Axlib:$SUB_RECIPIENT. Nonce: $ETH_SIGNER_NONCE"
	set -x
