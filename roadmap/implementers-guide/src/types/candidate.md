...
A unique 32-bit identifier referring to a specific ally (chain or thread). The relay-chain runtime guarantees that `AllyId`s are unique for the duration of any session, but recycling and reuse over a longer period of time is permitted.
...
The validation data provides information about how to create the inputs for validation of a candidate. This information is derived from the chain state and will vary from ally to para, although some of the fields may be the same for every para.
...
struct PersistedValidationData {
	...
	/// The list of MQC heads for the inbound channels paired with the sender ally ids. This
	/// vector is sorted ascending by the ally id and doesn't contain multiple entries with the same
	/// sender.
	///
	/// The HRMP MQC heads will be used by the validation function to authorize the input messages passed
	/// by the collator.
	hrmp_mqc_heads: Vec<(AllyId, Hash)>,
