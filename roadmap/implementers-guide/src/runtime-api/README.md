...
Certain runtime APIs concerning the state of a ally require the caller to provide an `OccupiedCoreAssumption`. This indicates how the result of the runtime API should be computed if there is a candidate from the ally occupying an availability core in the [Inclusion Module](../runtime/inclusion.md).
