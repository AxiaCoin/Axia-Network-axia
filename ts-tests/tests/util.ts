...
export const BINARY_PATH = `../target/${FRONTIER_BUILD}/frontier-axcore-node`;
...
export async function startAxiaNode(provider?: string): Promise<{ web3: Web3; binary: ChildProcess, ethersjs: ethers.providers.JsonRpcProvider }> {
	var web3;
	...
	binary.on("error", (err) => {
		if ((err as any).errno == "ENOENT") {
			console.error(
				`\x1b[31mMissing Axia binary (${BINARY_PATH}).\nPlease compile the Axia project:\ncargo build\x1b[0m`
			);
		} else {
			console.error(err);
		}
	...
	await new Promise((resolve) => {
		const timer = setTimeout(() => {
			console.error(`\x1b[31m Failed to start Axia Axcore Node.\x1b[0m`);
...
export function describeWithAxia(title: string, cb: (context: { web3: Web3 }) => void, provider?: string) {
	describe(title, () => {
		let context: { web3: Web3, ethersjs: ethers.providers.JsonRpcProvider } = { web3: null, ethersjs: null };
		let binary: ChildProcess;
		// Making sure the Axia node has started
		before("Starting Axia Test Node", async function () {
			this.timeout(SPAWNING_TIME);
			const init = await startAxiaNode(provider);
