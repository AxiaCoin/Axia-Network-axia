...
#[cfg(feature = "std")]
use axc_util_mem::{MallocSizeOf, MallocSizeOfOps};
...
pub use axc_scale_codec::Compact;
...
impl axc_scale_codec::Encode for CompactStatement {
	fn size_hint(&self) -> usize {
		// magic + discriminant + payload
		4 + 1 + 32
	}

	fn encode_to<T: axc_scale_codec::Output + ?Sized>(&self, dest: &mut T) {
		dest.write(&BACKING_STATEMENT_MAGIC);
		CompactStatementInner::from(self.clone()).encode_to(dest)
	}
}

impl axc_scale_codec::Decode for CompactStatement {
	fn decode<I: axc_scale_codec::Input>(
		...
		if maybe_magic != BACKING_STATEMENT_MAGIC {
			return Err(axc_scale_codec::Error::from("invalid magic string"))
		}
