...
fn mint_and_claim_should_works() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(AxiaClaims::set_bridge_account_elastic(
      Origin::root(),
      BridgeNetworks::BSC,
      1
    ));
    ...
    // Set claim limit = 100
    assert_ok!(AxiaClaims::set_claim_limit_elastic(
      Origin::root(),
      BridgeNetworks::BSC,
      100
    ));
    ...
    assert_eq!(AxiaClaims::elastic_claim_limits(BridgeNetworks::BSC), 100);
    assert_eq!(
      AxiaClaims::elastic_claim_limits(BridgeNetworks::Ethereum),
      200
    );
    ...
    assert_ok!(AxiaClaims::mint_claim_elastic(
      Origin::signed(1),
      BridgeNetworks::BSC,
      tx_hash.clone(),
      eth_addr.clone(),
    ...
    assert_eq!(
      AxiaClaims::elastic_claims(BridgeNetworks::BSC, &tx_hash),
      Some((eth_addr.clone(), 100, false))
    ); // new tx
    assert_eq!(AxiaClaims::elastic_claim_limits(BridgeNetworks::BSC), 0);
    ...
    // Pay AXCs to the TEST account:0100000000000000
    let sig = get_legal_eth_sig();
    assert_eq!(Balances::free_balance(1), 0);
    assert_ok!(AxiaClaims::claim_elastic(
      Origin::none(),
      BridgeNetworks::BSC,
      1,
      tx_hash.clone(),
...
fn ethereum_mint_and_claim_should_works() {
  new_test_ext().execute_with(|| {
    ...
    // claims and limit should updated
    assert_eq!(
      AxiaClaims::elastic_claims(BridgeNetworks::Ethereum, &tx_hash),
      Some((eth_addr.clone(), 100, false))
    ); // new tx
    assert_eq!(
      AxiaClaims::elastic_claim_limits(BridgeNetworks::Ethereum),
...
fn mint_fee_should_work() {
  new_test_ext().execute_with(|| {
    assert_ok!(AxiaClaims::set_bridge_fee_elastic(
      Origin::root(),
      BridgeNetworks::BSC,
      1000,
      0
    ...
    assert_eq!(
      AxiaClaims::elastic_claim_limits(BridgeNetworks::BSC),
      2000
    );
    ...
    assert_eq!(Balances::free_balance(AxiaClaims::account_id()), 1000);
  })
}
...
fn change_bridger_should_work() {
  new_test_ext().execute_with(|| {
    assert_noop!(
      AxiaClaims::set_bridge_account_elastic(Origin::signed(1), BridgeNetworks::BSC, 1),
...
fn non_exist_tx_should_fail() {
  new_test_ext().execute_with(|| {
    ...
    let sig = get_legal_eth_sig();
    assert_noop!(
      AxiaClaims::claim_elastic(Origin::none(), BridgeNetworks::BSC, 1, tx_hash, sig),
      Error::<Test>::TxNotMinted
    );
  });
}
...
fn illegal_sig_claim_should_fail() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      AxiaClaims::set_bridge_account_elastic(Origin::signed(2), BridgeNetworks::BSC, 1),
      DispatchError::BadOrigin
    );
    assert_ok!(AxiaClaims::set_bridge_account_elastic(
      Origin::root(),
...
fn claim_limit_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(AxiaClaims::elastic_claim_limits(BridgeNetworks::BSC), 10);
...
fn burn_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(AxiaClaims::burn_elastic(
      Origin::signed(4),
      BridgeNetworks::BSC,
      eth_addr,
      40
    ...
    assert_noop!(
      AxiaClaims::burn_elastic(Origin::signed(5), BridgeNetworks::BSC, eth_addr, 100),
...
fn burn_fee_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      AxiaClaims::burn_elastic(Origin::signed(4), BridgeNetworks::BSC, eth_addr, 20),
      Error::<Test>::InvalidAmount
    );
    ...
    // module has the fee deposited
    assert_eq!(Balances::free_balance(AxiaClaims::account_id()), 30);
  });
}
...
fn runtime_upgrade_should_copy_data() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(AxiaClaims::pallet_storage_version(), 1);
    ...
    assert_eq!(AxiaClaims::mint_fee(), 1000u64.some());
    assert_eq!(AxiaClaims::burn_fee(), 500u64.some());
    assert_eq!(AxiaClaims::claim_limit(), 2000);
    assert_eq!(AxiaClaims::claims(get_legal_tx_hash()).is_some(), true);
    assert_eq!(
      AxiaClaims::elastic_claims(BridgeNetworks::BSC, get_legal_tx_hash()).is_none(),
      true
    );

    // elastic config should be empty/default values
    assert_eq!(
      AxiaClaims::elastic_bridge_accounts(BridgeNetworks::BSC),
      None
    );
    assert_eq!(AxiaClaims::elastic_claim_limits(BridgeNetworks::BSC), 0);
    assert_eq!(AxiaClaims::bridge_fees(BridgeNetworks::BSC), (0, 0));

    // upgrade the data schema
    <AxiaClaims as OnRuntimeUpgrade>::on_runtime_upgrade();

    assert_eq!(AxiaClaims::pallet_storage_version(), 2);
    ...
    assert_eq!(AxiaClaims::bridge_fees(BridgeNetworks::BSC), (1000, 500));

    assert_eq!(
      AxiaClaims::elastic_claims(BridgeNetworks::BSC, get_legal_tx_hash()),
